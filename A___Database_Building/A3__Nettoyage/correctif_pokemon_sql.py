# Lors du scrap, certaines exceptions n'ont pas vu leurs talents leurs formes correctement scrapés ou  (env. 80 sur 1150)
...
Pokemon(db, 'Tauros').add_new_form(
    'Tauros de Paldéa (Race Combattive)',
...
Pokemon(db, 'Tauros de Paldéa (Race Combattive)').add_new_form(
    'Tauros de Paldéa (Race Flamboyante)',
...
# Ajout forme solaire
Pokemon(db, 'Morphéo (Normale)').add_new_form(
    'Morphéo (Solaire)',
...
##
### N.386 Deoxys
Pokemon(db, 'Deoxys').update(
    nom_pkmn='Deoxys (Normale)',
    nom_pkmn_us='Deoxys (Normal)'
)

# Ajout forme Attaque
Pokemon(db, 'Deoxys (Normale)').add_new_form(
    'Deoxys (Attaque)',
...
##
### N.393 Tiplouf
Pokemon(db, 'Tiplouf').update(
    talent_1='Torrent',
    talent_cache='Acharné (gen :8)/ Battant (gen 9:)'
)

# N.394 Prinplouf
Pokemon(db, 'Prinplouf').update(
    talent_1='Torrent',
    talent_cache='Acharné (gen :8)/ Battant (gen 9:)'
)

# N.395 Pingoléon
Pokemon(db, 'Pingoléon').update(
    talent_1='Torrent',
    talent_cache='Acharné (gen :8)/ Battant (gen 9:)'
)
...
##
### N.413 Cheniti
Pokemon(db, 'Cheniti').update(
    nom_pkmn='Cheniti (Cape Plante)',
    nom_pkmn_us='Burmy (Plant Cloak)'
)

# Ajout Cape Sable
Pokemon(db, 'Cheniti (Cape Plante)').add_new_form(
    'Cheniti (Cape Sable)',
...
# Ajout Cape Plante
Pokemon(db, 'Cheniselle (Cape Sable)').add_new_form(
...
# Ajout forme Chaleur
Pokemon(db, 'Motisma (Normale)').add_new_form(
    'Motisma (Chaleur)',
...
Pokemon.create(db, 
    ...
    url_cri='/images/0/0d/Cri_0483_HOME.ogg',
    type_1='Acier',
    type_2='Dragon',
    taille_m=5.4,
    poids_kg=683.0,
    ...
    taux_de_capture='3',
    cycle_eclosion=120,
    groupe_oeuf_1='Inconnu',
    points_effort='+3 Att. Spé',
    points_exp='220 Exp.',
    exp_niv100='1 250 000 exp',
    pv=100,
    attaque=120,
    defense=120,
...
Pokemon(db, 'Dialga (Normale)').add_new_form(
    'Dialga (Originelle)',
...
Pokemon.create(db, 
    ...
    url_cri='/images/e/ef/Cri_0484_HOME.ogg',
    type_1='Acier',
    type_2='Eau',
    taille_m=4.2,
    poids_kg=336.0,
...
Pokemon(db, 'Palkia (Normale)').add_new_form(
    'Palkia (Originelle)',
...
Pokemon.create(db, 
    ...
    url_cri='/images/d/d6/Cri_0487_HOME.ogg',
    type_1='Spectre',
    type_2='Dragon',
    taille_m=4.5,
    poids_kg=750.0,
...
Pokemon(db, 'Giratina (Alternative)').add_new_form(
    'Giratina (Originelle)',
...
Pokemon.create(db, 
    ...
    url_cri='/images/7/7a/Cri_0492_HOME.ogg',
    type_1='Plante',
    taille_m=0.2,
    poids_kg=2.1,
...
Pokemon(db, 'Shaymin (Terrestre)').add_new_form(
    'Shaymin (Céleste)',
...
Pokemon(db, 'Bargantua (Motif Blanc)').add_new_form(
...
Pokemon(db, 'Boréas (Avatar)').add_new_form(
    'Boréas (Totémique)',
...
Pokemon(db, 'Fulguris (Avatar)').add_new_form(
    'Fulguris (Totémique)',
...
Pokemon(db, 'Démétéros (Avatar)').add_new_form(
    'Démétéros (Totémique)',
...
Pokemon(db, 'Amphinobi').update(
    talent_1='Torrent',
    talent_cache='Protéen'
)

Pokemon(db, 'Amphinobi').add_new_form(
    'Amphinobi (Sacha)',
...
# Ajout Mistigrix (♀)
Pokemon(db, 'Mistigrix (♂)').add_new_form(
    'Mistigrix (♀)',
    nom_pkmn_us='Meowstic (♀)',
...
Pokemon.create(db, 
    ...
    url_cri='/images/5/59/Cri_0710_HOME.ogg',
    type_1='Spectre',
    type_2='Plante',
    taille_m=0.3,
    poids_kg=3.5,
    ...
    sexe='50 % femelle ; 50 % mâle',
    taux_de_capture=190,
    cycle_eclosion=20,
    groupe_oeuf_1='Amorphe',
    points_effort='+1 Défense',
    points_exp='67 exp.',
    exp_niv100='1 000 000 exp.',
    pv=44,
    attaque=66,
    defense=55,
    attaque_speciale=44,
...
Pokemon(db, 'Pitrouille (Mini)').add_new_form(
    'Pitrouille (Normale)',
...
Pokemon.create(db, 
    ...
    url_cri='/images/4/46/Cri_0711_HOME.ogg',
    type_1='Spectre',
    type_2='Plante',
    taille_m=0.7,
    poids_kg=9.5,
...
Pokemon(db, 'Banshitrouye (Mini)').add_new_form(
...
Pokemon.create(db, 
    ...
    url_cri='/images/7/75/Cri_0718_10_%25_HOME.ogg',
    type_1='Dragon',
    type_2='Sol',
    taille_m=1.2,
    poids_kg=33.5,
...
Pokemon(db, 'Zygarde (10%)').add_new_form(
    'Zygarde (50%)',
...
Pokemon.create(db, 
    ...
    url_cri='/images/4/4c/Cri_0720_HOME.ogg',
    type_1='Psy',
    type_2='Spectre',
    taille_m=0.5,
    poids_kg=9.0,
    ...
    cycle_eclosion=120,
    groupe_oeuf_1='Inconnu',
    points_effort='+3 Attaque Spéciale',
    points_exp='270 exp.',
    exp_niv100='1 250 000 exp.',
    pv=80,
    attaque=110,
...
Pokemon(db, 'Hoopa (Enchaîné)').add_new_form(
    'Hoopa (Déchaîné)',
...
Pokemon(db, 'Lougaroc (Diurne)').add_new_form(
    'Lougaroc (Nocturne)',
...
Pokemon.create(db, 
    ...
    url_cri='/images/0/0a/Cri_0746_Solitaire_HOME.ogg',
    type_1='Eau',
    taille_m=0.2,
    poids_kg=0.3,
    talent_1='Banc',
    ...
    taux_de_capture=60,
    cycle_eclosion=15,
    groupe_oeuf_1='Aquatique 2',
    points_effort='+1 PV',
    points_exp='61 exp.',
    exp_niv100='800 000 exp.',
    pv=45,
    attaque=20,
    defense=20,
    attaque_speciale=25,
...
Pokemon(db, 'Froussardine (Solitaire)').add_new_form(
    'Froussardine (Banc)',
...
Pokemon.create(db, 
    ...
    url_cri='/images/0/05/Cri_0774_HOME.ogg',
    type_1='Roche',
    type_2='Vol',
    taille_m=0.3,
    poids_kg=40.0,
    ...
    taux_de_capture=30,
    cycle_eclosion=25,
    groupe_oeuf_1='Minéral',
    points_effort='+1 Défense; +1 Défense Spéciale',
    points_exp='154 exp.',
    exp_niv100='1 059 860 exp.',
    pv=60,
    attaque=60,
...
Pokemon(db, 'Salarsen (Aïgue)').add_new_form(
    'Salarsen (Grave)',
...
Pokemon(db, 'Salarsen Gigamax').update(
    talent_1='Punk Rock',
...
Pokemon(db, 'Wimessir (♂)').add_new_form(
    'Wimessir (♀)',
    nom_pkmn_us='Indeedee (♀)',
...
Pokemon.create(db, 
    ...
    url_cri='/images/b/b8/Cri_0888_H%C3%A9ros_Aguerri_HOME.ogg',
    type_1='Fée',
    taille_m=2.8,
    poids_kg=110.0,
...
Pokemon(db, 'Zacian (Héros Aguerri)').add_new_form(
    'Zacian (Épée Suprême)',
...
Pokemon.create(db, 
    ...
    url_cri='/images/5/50/Cri_0889_H%C3%A9ros_Aguerri_HOME.ogg',
    type_1='Combat',
    taille_m=2.9,
    poids_kg=210.0,
...
Pokemon(db, 'Zamazenta (Héros Aguerri)').add_new_form(
...
Pokemon.create(db, 
    ...
    url_cri='/images/2/28/Cri_0901_HOME.ogg',
    type_1='Sol',
    type_2='Normal',
    taille_m=2.4,
    poids_kg=290.0,
...
Pokemon(db, 'Ursaking (Normale)').add_new_form(
...
Pokemon(db, 'Amovénus (Avatar)').add_new_form(
    'Amovénus (Totémique)',
...
Pokemon(db, 'Fragroin (♂)').add_new_form(
    'Fragroin (♀)',
    nom_pkmn_us='Oinkologne (♀)',
...
Pokemon.create(db, 
    ...
    url_cri='/images/7/77/Cri_0925_Trois_HOME.ogg',
    type_1='Normal',
    taille_m=0.3,
    poids_kg=2.3,
    talent_1='Garde-Ami',
    ...
    cycle_eclosion=10,
    groupe_oeuf_1='Terrestre',
    groupe_oeuf_2='Féerique',
    points_effort='+2 Vitesse',
    points_exp='165 exp.',
    exp_niv100='800 000 exp.',
    pv=74,
    attaque=75,
...
Pokemon(db, 'Famignol (Famille de Trois)').add_new_form(
...
Pokemon.create(db, 
    ...
    url_cri='/images/f/f0/Cri_0963_HOME.ogg',
    type_1='Eau',
    taille_m=1.3,
    poids_kg=60.2,
...
Pokemon(db, 'Superdofin (Ordinaire)').add_new_form(
    'Superdofin (Super)',
...
Pokemon.create(db, 
    ...
    url_cri='/images/5/5a/Cri_0982_HOME.ogg',
    type_1='Normal',
    taille_m=3.6,
    poids_kg=39.2,
    talent_1='Sérénité',
    ...
    points_effort='+2 PV',
    points_exp='182 exp.',
    exp_niv100='1 000 000'
)

Pokemon(db, 'Deusolourdo (Double)').add_new_form(
    'Deusolourdo (Triple)',
...
Pokemon.create(db, 
    ...
    url_cri='/images/0/0d/Cri_0999_Coffre_HOME.ogg',
    type_1='Spectre',
    taille_m=0.3,
    poids_kg=5.0,
    talent_1='Phobique',
...
Pokemon(db, 'Mordudor (Coffre)').add_new_form(
    'Mordudor (Marche)',
...
Pokemon(db, 'Ogerpon (Masque Turquoise)').add_new_form(
...
Pokemon.create(db, 
    ...
    url_cri='/images/9/90/Cri_1024_HOME.ogg',
    type_1='Normal',
    taille_m=0.2,
    poids_kg=6.5,
...
Pokemon(db, 'Terapagos (Normale)').add_new_form(
